...
# @@@ this needs some rethinking (Djangocon 2012 specific and hacky as heck)
tue_morn_start = datetime.datetime(2012, 9, 3, 12, 0)  # 9AM Pacific
tue_morn_end = datetime.datetime(2012, 9, 3, 15, 20)  # 12:20PM Pacific
tue_after_start = datetime.datetime(2012, 9, 3, 16, 20)  # 1:20PM Pacific
tue_after_end = datetime.datetime(2012, 9, 3, 19, 40)  # 4:40PM Pacific

TUESDAY_MORNING = (tue_morn_start, tue_morn_end)
TUESDAY_AFTERNOON = (tue_after_start, tue_after_end)
        ...
        "tue": {
            "morning": {
                ...
                "presentations": Presentation.objects.filter(
                    slot__start=TUESDAY_MORNING[0]
                ).order_by("pk"),
            },
            "afternoon": {
                "slot": TUESDAY_AFTERNOON,
                "presentations": Presentation.objects.filter(
                    slot__start=TUESDAY_AFTERNOON[0]
                ).order_by("pk"),
            }
        }
    }
...
def schedule_user_bookmarks(request, user_id, user_hash):
    ...
    cal = Calendar()
    cal.add("prodid", "-//Djangocon US 2012 Bookmarks//djangocon.us//EN")
    cal.add("version", "2.0")
    cal.add("method", "REQUEST")
    cal.add("calscale", "GREGORIAN")
    cal.add("X-WR-CALNAME", "Djangocon US 2012 Bookmarks - %s" % user.username)
    ...
    for bookmark in bookmarks:
        ...
        if p.slot is not None:
            ...
            event["uid"] = str(p.pk) + "-2012.djangocon.us"
            cal.add_component(event)
    
    response = HttpResponse(cal.as_string(), content_type="text/calendar")
    response["Content-Disposition"] = "filename=djangocon.us2012-%s-bookmarks.ics" % user.username.encode("utf-8")
    return response
...
def schedule_json(request):
    ...
    for slot in slots:
        try:
            tags = []
            tags.append(slot.presentation.kind.slug)
